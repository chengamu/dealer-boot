...
- `sys_menu`、`sys_role`、`sys_role_menu` 是平台统一维护的权限元数据，固定归属平台租户 `tenant_id = 1`；商家租户只读取授权结果，不创建租户私有菜单或角色。
- 这三张表跳过租户插件是为了让商家用户读取平台统一权限，不代表写入时会自动补平台租户。所有运行时写入口必须在 Service 层校验平台身份，并显式写入 `tenant_id = 1`。
- `sys_user_role` 仍归属实际用户所在租户；平台固定角色可以授权给不同商家租户的用户，不能把角色元数据归属和用户授权关系归属混为一谈。
