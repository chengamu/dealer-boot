...
正式页面不是把 H5 三个文件搬进系统，也不是新增一个笼统的“产品中心”大菜单。H5 的三个视图只是业务视角，正式后台入口按 `基础资料`、`配置与价格`、`发布与应用` 三个一级菜单承载，二级菜单直接进入业务页面。

| 正式能力 | 建议落点 | 说明 |
| --- | --- | --- |
| 配置录入工作台 | `配置与价格 / 配置模板` | 复用现有列表、表格、抽屉、上传、权限、i18n，自定义三栏内容区 |
| 销售只读总览 | `发布与应用 / 销售只读总览` | 从已发布配置自动生成，不给销售编辑入口 |
| 下单模拟/测试器 | `配置与价格 / 报价预览` 或配置模板内测试面板 | 用于发布前测试，不替代正式报价/订单页面 |
