...
| 租户上下文 | `bocoo.tenant`、`LoginHelper.getTenantId()`、数据权限拦截 | 产品、价格、资料、发布数据默认纳入租户边界，平台资料按现有忽略/平台租户规则评估 |
...
| 后期拆服务 | 产品中心发布事件或同步只读 read model 到订单系统 |
| ERP/MES 消费 | 只读发布版本、BOM/用量/物料映射，不直接写产品中心核心表 |

不建议多个系统共享同一个可写数据库。未来拆分后，产品中心写自己库，订单、ERP、MES 通过 API、事件、同步表或快照消费。
...
当前项目已启用租户能力，配置中心必须明确哪些数据是平台级，哪些数据是租户级。否则后期会出现“商户改了公共配置影响其他商户”的风险。

建议第一版按下面口径：

| 数据 | 默认归属 | 说明 |
| --- | --- | --- |
| 产品分类、产品模型、公共问题组、标准组件、标准资料 | 平台租户 / 共享主数据 | 由平台产品运营维护，商户、订单、ERP、MES 只读或引用 |
| 销售变体、配置模板发布版本 | 平台租户为主 | 如果以后允许商户定制，复制为商户草稿，不直接改平台发布版 |
| 价格方案 | 平台价格或商户价格都可能存在 | 第一版字段预留 `tenantId + scopeType`，发布检查按当前商品引用的价格范围判断 |
| 配置草稿、缺口待办、测试用例 | 创建者所在租户 | 平台创建平台草稿，商户创建商户草稿 |
| 报价/订单快照 | 当前商户租户 | 快照锁定当时配置、价格、资料版本，不受后续平台修改影响 |
| Agent 任务和日志 | 当前登录用户和租户 | 不能跨租户读取资料或生成草稿 |

如果某些基础表需要加入 `bocoo.tenant.ignore-tables`，必须在技术设计阶段逐表评估；不要默认全部忽略租户。
