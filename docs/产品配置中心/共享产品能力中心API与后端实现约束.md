...
读取来源优先 `pc_read_model`、`pc_gap_task`、`pc_event_outbox`，不要实时 join 全部编辑表。
...
| GET | `/product-capability/published-packages/list` | `product:publish:check` | 发布包分页 |
| GET | `/product-capability/published-packages/{packageId}` | `product:publish:check` | 发布包详情 |
...
| 方法 | 路径 | 权限 | 说明 |
| --- | --- | --- | --- |
| GET | `/product-capability/sales/products/list` | `product:sales:view` | 销售只读产品列表 |
| GET | `/product-capability/sales/products/{productCode}` | `product:sales:view` | 销售只读总览详情 |
| POST | `/product-capability/sales/products/{productCode}/quote-preview` | `product:sales:view` | 快捷报价预览 |
| GET | `/product-capability/sales/products/{productCode}/documents` | `product:sales:view` | 客户可见资料 |
| POST | `/product-capability/orders/build-snapshot` | 内部服务权限 | 构建订单产品能力快照 |

订单、报价保存前必须调用后端 `build-snapshot` 或等价 Service。快照结果保存当时的 code、name、version、单位、币种、价格明细、BOM、资料引用，不只保存 ID。
...
| GET | `/product-capability/event-outbox/list` | `product:center:view` | 同步事件分页 |
| POST | `/product-capability/event-outbox/{eventId}/retry` | `product:center:handle` | 重试同步 |
...
| 订单快照 | 订单历史必须自包含 | `SnapshotBuildEngine` + 订单侧 Service |
...
| BO | 请求入参，优先按生成器继承 `BaseBo<Entity>`，含校验注解，不暴露审计字段给前端写 |
...
product:publish:check/approve/publish
product:sales:view
```
...
| 订单快照 | `SnapshotBuildEngine` / 订单侧 Service |
