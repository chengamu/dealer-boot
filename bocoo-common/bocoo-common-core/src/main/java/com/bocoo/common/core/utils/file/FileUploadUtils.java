...
import com.bocoo.common.core.exception.file.FileNameLengthLimitExceededException;
import com.bocoo.common.core.exception.file.FileSizeLimitExceededException;
...
public class FileUploadUtils implements ApplicationContextAware
{
    ...
    public static String getDefaultBaseDir()
    {
        if (defaultBaseDir == null) {
            ...
            if (config != null) {
                defaultBaseDir = config.getProfile();
            }
        }
    ...
    {
        return StringUtils.format("{}/{}.{}", DateUtils.datePath(), IdUtil.fastSimpleUUID(), getExtension(file));
    }
...
public static final String getPathFileName(String uploadDir, String fileName) throws IOException
{
    BocooConfig config = getBocooConfig();
    if (config == null) {
        throw new IOException("无法获取系统配置");
    }

    String profile = config.getProfile();
    if (profile == null) {
        throw new IOException("系统配置中未设置上传路径");
    }

    if (uploadDir.startsWith(profile)) {
        String relativePath = uploadDir.substring(profile.length());
        // 标准化路径分隔符
