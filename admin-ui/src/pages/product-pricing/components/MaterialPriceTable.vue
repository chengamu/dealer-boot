<template>
  <section class="price-section">
    <div class="price-section__header">
      <div>
        <h3>{{ t('productCenter.pricing.materialRules') }}</h3>
        <p>{{ t('productCenter.pricing.materialRulesHint') }}</p>
      </div>
    ...
    <el-table :data="rows" border class="price-rule-table" height="calc(100vh - 410px)" row-key="priceMaterialId" @selection-change="selectedRows = $event">
      <el-table-column type="selection" width="48" align="center" />
      <el-table-column type="index" :label="t('common.index')" width="58" align="center" />
      <el-table-column prop="attributeGroupNameCn" :label="t('productCenter.formulaSetup.attributeGroup')" width="104" />
      ...
      <el-table-column prop="materialNameCn" :label="t('productCenter.formulaSetup.materialName')" min-width="280" show-overflow-tooltip />
      ...
      <el-table-column :label="t('common.operate')" width="128" align="center" fixed="right">
        <template #default="{ row }">
          <el-button v-if="editable" v-hasPermi="['product:pricing:edit']" link type="primary" @click="$emit('open-rules', row)">
...
defineEmits<{
  ...
  batch: [rows: PriceMaterialVO[]]
  'open-rules': [row: PriceMaterialVO]
}>()
...
const props = defineProps<{
  rows: PriceMaterialVO[]
  rules: MaterialPriceRule[]
  editable?: boolean
  options?: ProductFormulaOptionVO[]
  optionValues?: ProductFormulaOptionValueVO[]
}>()
const selectedRows = ref<PriceMaterialVO[]>([])

watch(() => props.rows, () => {
  selectedRows.value = []
})

function summaryRows(row: PriceMaterialVO) {
  const rowRules = props.rules.filter(rule => String(rule.priceMaterialId || '') === String(row.priceMaterialId || ''))
  if (!rowRules.length) {
    return [{
      key: 'missing',
