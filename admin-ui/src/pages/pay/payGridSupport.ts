...
import type { BusinessOrigin } from '@/api/pay'
...
export function useFinanceSubjectFilter(query: SubjectQuery) {
...
export function syncSubjectByOrigin<T extends SubjectQuery>(target: T, source: Ref<BusinessOrigin | ''>) {
  watch(
    source,
    (value) => {
      target.businessOrigin = value
      target.subjectId = ''
    },
    { immediate: true }
  )
