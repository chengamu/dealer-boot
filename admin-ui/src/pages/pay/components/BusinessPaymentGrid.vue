...
import { businessPaymentApi, type PayOrder, type PayOrderQuery, type PayOrderStatus } from '@/api/pay'
import { checkPermi } from '@/utils/permission'
import { withUtcDateRange } from '@/utils/datetime'
import PaymentDetailDrawer from './PaymentDetailDrawer.vue'
import { currentMonthDateRange, formatMinute } from '../payGridSupport'
import { methodText, minorMoney, statusText, statusType } from '../payPresentation'

const emit = defineEmits<{ checkout: [salesDocumentId: string] }>()
const { t } = useI18n()
...
const query = reactive<PayOrderQuery>({ pageNum: 1, pageSize: 20, keyword: '', channelCode: '', status: '' })

async function load() {
  loading.value = true
  try {
    const response = await businessPaymentApi.list(withUtcDateRange({ ...query }, dateRange.value))
...
function reset() {
  Object.assign(query, { pageNum: 1, pageSize: 20, keyword: '', channelCode: '', status: '' })
  dateRange.value = currentMonthDateRange()
  void load()
}

function openDetail(row: PayOrder) {
  detailRef.value?.open(row, 'business')
}

function openCheckout(row: PayOrder) {
  if (!row.salesDocumentId) return
  emit('checkout', String(row.salesDocumentId))
}
...
function rowActions(row: PayOrder): AdminTableAction[] {
  return [
    ...
    {
      label: t('pay.checkout.action'),
      permission: 'pay:order:submit',
      hidden: row.status !== 0 || !row.salesDocumentId || !checkPermi(['pay:order:submit']),
