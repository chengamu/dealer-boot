<template>
  <div class="app-container merchant-table-page">
    <el-form v-show="showSearch" ref="queryRef" :model="queryParams" :inline="true" class="merchant-table-page__search">
      <el-form-item :label="t('customer.tenantId')" prop="tenantId"><el-input v-model="queryParams.tenantId" clearable style="width: 130px" @change="handleTenantChange" @keyup.enter="handleQuery" /></el-form-item>
      <el-form-item :label="t('merchantProfile.merchantName')" prop="merchantId">
        <el-select v-model="queryParams.merchantId" clearable filterable style="width: 190px">
      ...
      <el-form-item :label="t('customer.owner')" prop="ownerUserId">
        <el-select v-model="queryParams.ownerUserId" clearable filterable :disabled="!queryParams.tenantId" style="width: 170px">
    ...
    <el-table v-loading="loading" :data="rows" border class="merchant-table-page__table">
      ...
      <el-table-column :label="t('customer.tenantId')" prop="tenantId" width="110" />
      <el-table-column :label="t('merchantProfile.merchantName')" prop="merchantName" min-width="160" :show-overflow-tooltip="true" />
    ...
    <AdminDrawer v-model="detailOpen" :title="t('customer.detailTitle')" size="560px" variant="detail">
      <el-descriptions v-if="detail" :column="1" border>
        <el-descriptions-item :label="t('customer.tenantId')">{{ detail.tenantId }}</el-descriptions-item>
        <el-descriptions-item :label="t('merchantProfile.merchantName')">{{ detail.merchantName || '-' }}</el-descriptions-item>
...
async function loadOwners() {
  ...
  const res = await listPlatformCustomerOwnerOptions(queryParams.tenantId)
