  ...
  <div class="quote-workbench" v-loading="loading">
    <header class="quote-workbench__topbar">
      ...
      <div class="quote-workbench__actions">
        ...
        <el-button v-if="quote.status === 'CONFIRMED'" v-hasPermi="['customer:quote:edit']" type="danger" plain icon="CircleClose" @click="voidQuote">{{ t('customer.quote.action.void') }}</el-button>
      </div>
    </header>
...
const { quote, rows, customers, owners, saleProducts, setups, loading, loadingProductId, loadDependencies, loadQuote, loadSetup, addLine, duplicateLine, removeLine } = data
...
async function duplicateAndExpand(row: QuoteWorkbenchItem) { const copy = duplicateLine(row); await nextTick(); lineGridRef.value?.expand(copy) }
