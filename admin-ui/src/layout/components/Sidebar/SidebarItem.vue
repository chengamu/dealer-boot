...
import type { RouteMeta } from '@/types/api'
...
const onlyOneChild = ref<RouteItem>({ path: '' });
...
function resolveLinkPath(routePath: string, routeQuery?: string) {
  return routeQuery ? resolvePath(routePath, routeQuery) : resolvePath(routePath)
}
