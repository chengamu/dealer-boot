...
public class SysLoginService {
    ...
    public Map<String, Object> googleLogin(String credential) {
        String googleEmail = googleAuthService.verifyCredential(credential);
        SysUserVo user = findUserByEmail(googleEmail);
        if (ObjectUtil.isNotNull(user)) {
            user = checkLoadedUser(user, googleEmail);
            LoginUser loginUser = buildLoginUser(user);
            LoginHelper.loginByDevice(loginUser, DeviceType.PC);
        ...
        return buildGoogleApplyStatus(googleEmail);
    }
